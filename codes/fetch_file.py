...
def fetch_file_from_line(file_lines: list[Tuple[str, int]], context_lines: int = 12, max_gap: int = 0) -> str:
    def extract_context(
        file_lines: list[Tuple[str, int]], context_lines: int = 12
        ...
        for relative_path, line_idx in file_lines:
            ...
            start_idx: int = max(1, line_idx - context_lines)
            end_idx: int = min(num_lines, line_idx + context_lines)
    ...
    context_snippets: List[List[List[Tuple[int, str]]]] = extract_context(file_lines, context_lines=context_lines)
    ...
    # Header
    output.write("Extracted stated files successfully.\n\n")
    output.write("Extraction Results (by file, merging any overlapping context):\n\n")
