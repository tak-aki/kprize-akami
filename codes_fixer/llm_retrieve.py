...
from .config import BATCH_SIZE, model_7b
...
def get_llm_retrieval(problem_statement: str, codebase_path: str, candidate_file_batch: List[dict]):
    ...
    sampling_params: SamplingParams = SamplingParams(
        ...
        max_tokens=model_7b["MAX_TOKENS"],
    )
    ...
    for candidate_file in candidate_file_batch:
        ...
        while count_tokens(json.dumps(prompt_json), model_7b["tokenizer"]) > (model_7b["MAX_MODEL_LEN"] - 100): # 100 is a buffer
            logger.info(
                f"Exceeding token limit ({count_tokens(json.dumps(prompt_json), model_7b['tokenizer'])} > {model_7b['MAX_MODEL_LEN']}), remove last file and remaining files: {len(prompt_json['input']['retrieved file documentations'])-1}"
            )
    ...
    prompt_texts: List[str] = [
        model_7b['tokenizer'].apply_chat_template(conversation=messages, tokenize=False, add_generation_prompt=True)  # type: ignore
        for messages in list_of_messages
    ]

    logger.info(f"prompt_texts token length {[count_tokens(text, model_7b['tokenizer']) for text in prompt_texts]}")
    request_outputs: list[RequestOutput] = model_7b["llm"].generate(prompt_texts, sampling_params=sampling_params)
    response_texts_from_inference: List[str] = [request_output.outputs[0].text for request_output in request_outputs]
    logger.info(f"response_texts_from_inference token length : {[count_tokens(text, model_7b['tokenizer']) for text in response_texts_from_inference]}")
